...
import com.facebook.presto.orc.reader.*;
...
import org.apache.spark.sql.execution.datasources.orc.streamreaders.DecimalStreamReader;
...
public class SepecificOrcRecordReaderBase
{
    ...
    public SepecificOrcRecordReaderBase(
        ...
        throws IOException
    {
        ...
        orcDataSource = wrapWithCacheIfTinyStripes(orcDataSource, this.stripes,
        ...
        streamReaders = createStreamReaders(orcDataSource, types, hiveStorageTimeZone,
            presentColumnsAndTypes.build());
    }

    private static boolean splitContainsStripe(
        long splitOffset, long splitLength, StripeInformation stripe)
    {
        ...
        OrcPredicate predicate)
    {
        ...
        return predicate.matches(stripe.getNumberOfRows(),
                getStatisticsByColumnOrdinal(rootStructType,
                stripeStats.get().getColumnStatistics()));
    }

    @VisibleForTesting
    static OrcDataSource wrapWithCacheIfTinyStripes(
        ...
        DataSize maxReadSize)
    {
        ...
        return new CachingOrcDataSource(dataSource,
            LinearProbeRangeFinder.createTinyStripesRangeFinder(stripes, maxMergeDistance,
                maxReadSize));
    }
    ...
    public int nextBatch() throws IOException
    {
        ...
        currentBatchSize = Ints.checkedCast(Math.min(OrcReader.MAX_BATCH_SIZE,
    ...
    private void advanceToNextStripe() throws IOException
    {
        ...
        Stripe stripe = stripeReader.readStripe(stripeInformation,
            currentStripeSystemMemoryContext);
        if (stripe != null) {
    ...
    private static StreamReader[] createStreamReaders(
        ...
        Map<Integer, Type> includedColumns)
    {
        List<StreamDescriptor> streamDescriptors =
        ...
        for (int columnId = 0; columnId < rowType.getFieldCount(); columnId++) {
            if (includedColumns.containsKey(columnId)) {
                ...
                streamReaders[columnId] =
                    createStreamReader(streamDescriptor, hiveStorageTimeZone);
            }
        }
        ...
        OrcDataSource dataSource)
    {
        ...
        if (type.getOrcTypeKind() == OrcTypeKind.STRUCT) {
            for (int i = 0; i < type.getFieldCount(); ++i) {
                nestedStreams.add(createStreamDescriptor(parentStreamName,
                        type.getFieldName(i), type.getFieldTypeIndex(i), types, dataSource));
            }
        }
        else if (type.getOrcTypeKind() == OrcTypeKind.LIST) {
            nestedStreams.add(createStreamDescriptor(parentStreamName, "item",
                    type.getFieldTypeIndex(0), types, dataSource));
        }
        else if (type.getOrcTypeKind() == OrcTypeKind.MAP) {
            nestedStreams.add(createStreamDescriptor(parentStreamName, "key",
                    type.getFieldTypeIndex(0), types, dataSource));
            nestedStreams.add(createStreamDescriptor(parentStreamName, "value",
                    type.getFieldTypeIndex(1), types, dataSource));
        }
        return new StreamDescriptor(parentStreamName, typeId, fieldName,
                type.getOrcTypeKind(), dataSource, nestedStreams.build());
    }

    private static Map<Integer, ColumnStatistics> getStatisticsByColumnOrdinal(
        OrcType rootStructType,
        List<ColumnStatistics> fileStats)
    {
    ...
    public static class LinearProbeRangeFinder
        implements CachingOrcDataSource.RegionFinder
    {
            ...
            DataSize maxReadSize)
        {
            ...
            List<DiskRange> scratchDiskRanges = stripes.stream()
                    .map(stripe -> new DiskRange(stripe.getOffset(),
                        Ints.checkedCast(stripe.getTotalLength())))
                    .collect(Collectors.toList());
            List<DiskRange> diskRanges = mergeAdjacentDiskRanges(scratchDiskRanges,
                maxMergeDistance, maxReadSize);
