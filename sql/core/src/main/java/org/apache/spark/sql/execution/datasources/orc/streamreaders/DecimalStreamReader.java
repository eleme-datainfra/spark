...
package org.apache.spark.sql.execution.datasources.orc.streamreaders;
...
import org.apache.spark.sql.execution.datasources.orc.stream.UnboundedIntegerStream;
import org.apache.spark.sql.execution.datasources.orc.type.DecimalType;
import org.apache.spark.sql.execution.datasources.orc.type.LongDecimalType;
import org.apache.spark.sql.execution.datasources.orc.type.ShortDecimalType;
        ...
        implements StreamReader
{
    ...
    @Nonnull
    private StreamSource<BooleanStream> presentStreamSource =
        missingStreamSource(BooleanStream.class);
    @Nullable
    private BooleanStream presentStream;

    @Nonnull
    private StreamSource<UnboundedIntegerStream> unboundedIntegerStreamSource =
            missingStreamSource(UnboundedIntegerStream.class);
    @Nullable
        ...
        if (readOffset > 0) {
            ...
            if (readOffset > 0) {
                if (unboundedIntegerStream == null) {
                    throw new OrcCorruptionException("Value is not null but unbounded " +
                        "integer stream is not present");
                }
                if (scaleStream == null) {
                    throw new OrcCorruptionException("Value is not null but scale " +
            ...
            throws IOException
    {
        if (presentStream == null) {
            ...
            if (decimalType.isShort()) {
                ...
                unboundedIntegerStream.nextBigIntegerVector(nextBatchSize,
                    longUnboundedIntegerVector);
            }
            ...
            if (nullValues != nextBatchSize) {
                ...
                if (decimalType.isShort()) {
                    unboundedIntegerStream.nextLongVector(nextBatchSize,
                        shortUnboundedIntegerVector, nullVector);
                }
                else {
                    unboundedIntegerStream.nextBigIntegerVector(nextBatchSize,
                        longUnboundedIntegerVector, nullVector);
                }
    ...
    {
        BlockBuilder builder = decimalType.createBlockBuilder(new BlockBuilderStatus(),
            nextBatchSize);

        if (decimalType.isShort()) {
            for (int i = 0; i < nextBatchSize; i++) {
                if (!nullVector[i]) {
                    long rescaledUnboundedInteger = rescale(shortUnboundedIntegerVector[i],
                            (int) scaleVector[i], decimalType.getScale());
            ...
            for (int i = 0; i < nextBatchSize; i++) {
                if (!nullVector[i]) {
                    BigInteger rescaledUnboundedInteger = rescale(longUnboundedIntegerVector[i],
                            (int) scaleVector[i], decimalType.getScale());
    ...
    public void startRowGroup(StreamSources dataStreamSources)
            throws IOException
    {
        presentStreamSource = dataStreamSources.getStreamSource(streamDescriptor, PRESENT,
            BooleanStream.class);
        unboundedIntegerStreamSource = dataStreamSources.getStreamSource(streamDescriptor, DATA,
                UnboundedIntegerStream.class);
        scaleStreamSource = dataStreamSources.getStreamSource(streamDescriptor, SECONDARY,
            LongStream.class);
