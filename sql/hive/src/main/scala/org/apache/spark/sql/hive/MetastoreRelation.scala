  ...
  @transient override lazy val statistics: Statistics = {
    catalogTable.stats.getOrElse(Statistics(
      sizeInBytes = {
        ...
        // ESPARK-14: Now we disable broadcast join for partitioned-table temporarily.
        val partitionKeysQlTable = hiveQlTable.getTTable.getPartitionKeys
        ...
        BigInt(
          ...
          if (partitionKeysQlTable != null && !partitionKeysQlTable.isEmpty) {
            sparkSession.sessionState.conf.defaultSizeInBytes
          } else if (totalSize != null && totalSize.toLong > 0L) {
            totalSize.toLong
